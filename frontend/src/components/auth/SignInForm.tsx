...
export default function SignInForm() {
  ...
  const goToDashboard = () => {
    ...
    navigate(from && from !== "/" && from !== "/signin" ? from : "/dashboard", { replace: true });
  };
  ...
  const handleEmailSignIn = async (event: FormEvent) => {
    ...
    try {
      ...
      if (isClerkAPIResponseError(err)) {
        setError(mapClerkSignInError(err, t));
      } else {
  ...
  const handleOAuth = async (strategy: "oauth_google" | "oauth_x") => {
    if (!isLoaded || !signIn) return;
    ...
    try {
      ...
      if (isClerkAPIResponseError(err)) {
        setError(err.errors[0]?.longMessage ?? t("auth.oauthError"));
      } else {
        ...
        <div>
          <div className="mb-5 sm:mb-8">
            <div className="mb-6 flex justify-center lg:hidden">
              <TipAnimatedLogo size="md" layout="stack" showWordmark showPlatformName showSlogan animate />
            </div>
            <h1 className="mb-2 font-semibold text-gray-800 text-title-sm dark:text-white/90 sm:text-title-md">
              {t("auth.signIn")}
            </h1>
            <p className="text-sm text-gray-500 dark:text-gray-400">{t("auth.signInDesc")}</p>
          </div>
          <div>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-5">
              ...
              <button
                type="button"
                disabled={!isLoaded}
                onClick={() => void handleOAuth("oauth_x")}
                className="inline-flex items-center justify-center gap-3 py-3 text-sm font-normal text-gray-700 transition-colors bg-gray-100 rounded-lg px-7 hover:bg-gray-200 hover:text-gray-800 disabled:cursor-not-allowed disabled:opacity-60 dark:bg-white/5 dark:text-white/90 dark:hover:bg-white/10"
              >
            ...
            <div className="mt-5 space-y-2">
              ...
              <p className="text-sm text-center text-gray-500 dark:text-gray-500 sm:text-start">
                <Link to="/accept-invitation" className="text-brand-500 hover:text-brand-600 dark:text-brand-400">
                  {t("auth.activationLink")}
                </Link>
                {" · "}
