...
export default function SignInForm() {
  ...
  const handleEmailSignIn = async (event: FormEvent) => {
    ...
    try {
      ...
      setError("Connexion incomplète. Vérifiez votre invitation ou contactez un administrateur.");
    } catch (err) {
      if (isClerkAPIResponseError(err)) {
        setError(err.errors[0]?.longMessage ?? err.errors[0]?.message ?? "Identifiants invalides");
      } else {
  ...
  const handleOAuth = async (strategy: "oauth_google" | "oauth_x") => {
    ...
    try {
      ...
      if (isClerkAPIResponseError(err)) {
        setError(err.errors[0]?.longMessage ?? "Erreur de connexion OAuth");
      } else {
        ...
        <div>
          <div className="mb-5 sm:mb-8">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Accès réservé aux membres Traumatec. Connectez-vous avec votre compte invité.
            </p>
          </div>
          <div>
            ...
            <div className="mt-5">
              <p className="text-sm font-normal text-center text-gray-700 dark:text-gray-400 sm:text-start">
                Pas d&apos;inscription publique. Contactez un administrateur pour obtenir un accès.
              </p>
            </div>
          </div>
        </div>
